...
lambdas = {'adv': 0.25, 'identity': 10000, 'match_gt': 1, 'vis_reg': .25, 'consist': 20}
...
outfit_embedder = embedder_model.embedder_b.cuda()
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in pbar:
        ...
        # embedding_1_o = outfit_embedder(output_1)
        # embedding_1_t = outfit_embedder(transfer_1)
        # embedding_1_p = prod_embedder(data['color'].cuda())
        ...
        embedding_2_o = outfit_embedder(output_2)
        embedding_2_t = outfit_embedder(transfer_2)
        embedding_2_p = prod_embedder(data['color2'].cuda())
        ...
        hinge_identity = torch.mean(nn.ReLU()(embedding_mse(mse_criterion, embedding_2_o, embedding_2_p) - embedding_mse(mse_criterion, embedding_2_t, embedding_2_p)))
        ...
        total_loss = lambdas['identity'] * hinge_identity + \
        ...
        if single_gpu_flag(opt):
            ...
            board.add_scalar('loss/identity', hinge_identity.item(), step + 1)
            ...
            pbar.set_description('step: %8d, loss: %.4f, identity: %.4f, vis_reg: %.4f, mse: %.4f, consist: %.4f'
                  % (step + 1, total_loss.item(), hinge_identity.item(),
